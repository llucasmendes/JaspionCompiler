...
Vardeclaracao : Tipoespecificador ID PVIRG	{$$ = numero6($1,S_symbol($2));}
			| Tipoespecificador ID ABCOLC NUM FECOLC PVIRG {$$ = numero7($1,S_symbol($2),$4);}
;

Tipoespecificador : INT	{$$ = numero8();}
Tipoespecificador : VOID	{$$ = numero9();}
				  | STRINGE {$$ = numero60();}
;

Fundeclaracao  : Tipoespecificador ID ABPAREN Params FEPAREN Compostodecl	{$$ = numero10($1,S_symbol($2),$4,$6);}	
;
...
Param: Tipoespecificador ID {$$ = numero15($1,S_symbol($2);}
				| Tipoespecificador ID ABCOLC FECOLC {$$ = numero16($1,S_symbol($2));}
;
...
Var: ID		{$$ = numero36(S_symbol($1));}
	| ID ABCOLC Expressao FECOLC	{$$ = numero37(S_symbol($1),$3);}
;
...
Relacional: ME	{$$ = numero41();}
Relacional: MA	{$$ = numero401();}
Relacional: MAI	{$$ = numero411();}
Relacional: II	{$$ = numero402();}
Relacional: DIF	{$$ = numero412();}
;
...
Ativacao: ID ABPAREN Args FEPAREN	{$$ = numero54(S_symbol($1),$3);}
;
